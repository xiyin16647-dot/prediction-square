...
import { poolToPrice } from "@/lib/market";
...
export default async function MePage() {
  ...
  for (const p of positions) {
    ...
    if (s === "ACTIVE" || s === "FROZEN" || s === "RESOLVING") {
      const { yesPrice, noPrice } = poolToPrice(p.market.poolYes, p.market.poolNo);
      const value = new Decimal(p.yesShares.toString())
        .mul(yesPrice)
        .add(new Decimal(p.noShares.toString()).mul(noPrice));
      unrealized = unrealized.add(value);
    }
  }

  const totalPnl = balance + unrealized.toNumber() - REGISTER_BONUS;
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="mx-[18px] mb-4 p-4 bg-surface border border-line rounded-2xl font-sans">
          <div className="text-[11px] text-sub mb-1">
            累计盈亏 <span className="text-mut">（含未实现）</span>
          </div>
